...
class OLED_1inch3(framebuf.FrameBuffer):
    ...
    def show(self):
        # Assume self.buffer is a bytearray of length 1024 (128x64/8)

        # Send one 16-byte burst per column instead of 16 single-byte writes
        self.write_cmd(0xB0)  # keep your current addressing mode

        rot180 = (self.rotate == 180)
        buf_mv = memoryview(self.buffer)  # avoid extra copies

        for col in range(64):
            column = col if rot180 else (63 - col)

            # set lower/upper column address nibbles
            self.write_cmd(0x00 | (column & 0x0F))
            self.write_cmd(0x10 | (column >> 4))

            start = col * 16
            # burst out the 16 bytes for this column in one go
            self.write_data(buf_mv[start:start + 16])
